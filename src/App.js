...
import Movie from "./Movie";    // Movie.js로 따로 함수를 빼서 만듬 
import "./App.css";             // design css

class App extends React.Component {  // setstate 같은 component에 필요한 함수를 가져오기 위해 상속

  state = {                     // 왜 state를 사용하는가?, 기본 변수가 아니라, 
  ...
  getMovies = async () => {     // async으로 느리게 표현될 함수를 말한다.
    const {
      data: {
  ...
  componentDidMount() {         // 생명 주기 함수이다. component가 실행되면 먼저 이 함수가 실행된다.
    this.getMovies();
  }
      ...
      <section className="comtainer">
        {isLoading ? (
          ...
          <div className="movies">
            {movies.map( // map => 매개변수로 해당 array foreach 함수 실행
              (
...
export default App; // what meaing?
